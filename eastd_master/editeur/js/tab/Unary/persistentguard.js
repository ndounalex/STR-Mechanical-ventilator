const PERSISTENT_GUARD = "Persistent Guard",
	PERSISTENT_GUARD_SYMBOL = "~⇒";

class PersistentGuard extends Tab {
	...
	toHtml() {
		return PERSISTENT_GUARD_SYMBOL;
	}

	getType() {
		return PERSISTENT_GUARD;
	}
		...
		return {
			...
			createPlayMode: function() {
				...
				if (window.project.lastJsonObj !== undefined && window.project.lastJsonObj !== null) {
					...
					var guardObj = window.project.getJsonObjByType(window.project.lastJsonObj.top_level_astd, PERSISTENT_GUARD, parentLabel);
	...
	save() {
		...
		guard.class = Item.TYPES.PERSISTENT_GUARD;
	...
	copy(copiedParent, tab) {
		...
		return new CopiedPersistentGuart(copiedParent, tab.attributes, tab.code, tab.predicate);
	}
...
class CopiedPersistentGuart extends CopiedTab {
	...
	paste(parent) {
		return new PersistentGuard(window.id.get(), parent, this.attributes, this.code, this.predicate);
	}
}
