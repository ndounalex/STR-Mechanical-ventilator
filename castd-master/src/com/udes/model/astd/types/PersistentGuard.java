...
public class PersistentGuard extends UnaryASTD {
    private Condition guard;

    public PersistentGuard(String name,
    ...
    public Statement init(ArrayList<ASTD> callList, String lets) {
        ...
        // init state
        // init state is the inital ASTD from the body ASTD, line below not necessary so it is commented.
    ...
    public Condition _final(ArrayList<ASTD> callList) {
        ...
        ASTDTree.addAll(callList);
        ASTDTree.add(this);
        return new AndCondition(Arrays.asList(getBody()._final(ASTDTree)));
    }
    ...
    public Entry<List<Enum>, List<Record>> trans_type() {
        ...
        if(attrs != null)
            props.addAll(attrs);
        //astd state
        //the two lines below are not necessary and thus commented
        //String stateType = Conventions.getStateType(PermanentGuard.class);
        //props.add(new Variable(Conventions.getStateVar(PermanentGuard.class), stateType, null, getName()));
        //the line below is not necessary and thus commented
    ...
    public Entry<Condition, Statement> trans_event(Event e, Bool timed, ArrayList<ASTD> callList, String lets) {
        ...
        if(b != null) {
            ...
            if(eBody != null && !eBody.isEmpty()) {
                String gState = Conventions.getStateVar(PersistentGuard.class);
                // if-fi body block
                ...
                c1 = new AndCondition(Arrays.asList(
                        new AndCondition(Arrays.asList(guard, eBody.getKey()))));
                evtBody.setKey(c1);
    ...
    public Statement propertyMapping() {
        ...
        //the lines below are commented because they are not necessary
        ...
        Constants.EXEC_SCHEMA_PROPS = Constants.EXEC_SCHEMA_PROPS.replace(ILTranslator.USYMBOL_1,
                PersistentGuard.class.getSimpleName());
    ...
    private Statement fillGuardProperties(String index, String prfx) {
        //TODO: This function is probably broken due the change with permanent guard.
        List<Statement> stmtList = new ArrayList<>();

        String guardState = Conventions.getStateVar(PersistentGuard.class);
        ...
        stmtList.add(new AssignStatement(new Term(Conventions.ARRAY_ELEM.replace(ILTranslator.USYMBOL_2, index)
                ...
                new Term( "(std::string(" + Conventions.ARRAY_ELEM
                        .replace(ILTranslator.USYMBOL_2,
                                Conventions.getStateVar(PersistentGuard.class))
                        .replace(ILTranslator.USYMBOL_1, prfx + guardState)
                        + ").compare(\""
